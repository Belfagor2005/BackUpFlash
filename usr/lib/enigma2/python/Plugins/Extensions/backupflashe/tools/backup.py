...
from .bftools import (logdata, getboxtype, getimage_name, copylog)
...
class doBackUp(Screen):
    ...
    def doBackUp(self):
        ...
        if self.device_path:
            ...
            self.session.openWithCallback(self.dofinish, ProgressScreen, title=mytitle, cmdlist=cmdlist, imagePath=self.IMAGENAME1)
        else:
